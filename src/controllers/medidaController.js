...
function buscarUltimasMedidas(req, res){
    ...
    medidaModel.buscarIdSensor(idGalpao).then(function(resultado){
        ...
        if(resultado.length == 1){
            ...
            medidaModel.buscarUltimasMedidas(idSensor)
            .then(function(resultado){

                if(resultado.length > 0 && resultado.length <= 12){
...
function buscarEnderecosGalpao(req,res){
    var idEmpresa = req.params.idEmpresa;

    medidaModel.buscarEnderecosGalpao(idEmpresa).then(function (resultado){
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
...
function buscarGalpoes(req, res){

    var idEndereco = req.params.idEndereco;

    medidaModel.buscarGalpoes(idEndereco).then(function (resultado){
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
            res.status(204).send("Nenhum resultado encontrado!")
